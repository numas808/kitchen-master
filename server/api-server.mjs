...
const SEARCH_PROVIDER = (process.env.SEARCH_PROVIDER || 'serpapi').toLowerCase();
...
function normalizeSerpItem(item, index) {
  const imageUrl =
    ...
    `https://picsum.photos/seed/serp${index}/400/300`;

  return {
    id: `web-serp-${index}-${item.link || item.title || index}`,
    title: item.title || 'タイトル不明',
    description: item.snippet || '',
    imageUrl,
    sourceUrl: item.link || '',
...
async function searchBySerpApi(query) {
  ...
  target.searchParams.set('hl', 'ja');
  target.searchParams.set('gl', 'jp');
  target.searchParams.set('num', '8');
  target.searchParams.set('api_key', SERPAPI_KEY);
  ...
  if (!response.ok) {
    return {
      ...
      error: data?.error || `SerpAPIエラー (${response.status})`,
      items: [],
    };
  }
  ...
  return {
    ...
    status: 200,
    error: null,
    items: (data.organic_results || []).slice(0, 8).map(normalizeSerpItem),
  };
}
...
async function handleTodaysRecipe(req, res) {
  ...
  try {
    ...
    const serpResult = await searchBySerpApi(searchContext.searchQuery || requestText);

    if (!serpResult.ok) {
      sendJson(res, serpResult.status, {
    ...
    const selectedResults = serpResult.items.slice(0, 5);
    ...
    if (!selectedItem) {
      sendJson(res, 200, { error: '候補レシピが見つかりませんでした。', items: [], provider: 'serpapi' });
      return;
    }
